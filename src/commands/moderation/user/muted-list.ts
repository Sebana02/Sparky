...
export const command: ICommand = {
  ...
  execute: async (client: Client, inter: ChatInputCommandInteraction): Promise<void> => {
    ...
    if (!mutedRole) return await reply(inter, { content: commandLit.noMutedUsers, flags: MessageFlags.Ephemeral }, 2);
    ...
    if (!mutedMembers || mutedMembers.size === 0)
      return await reply(inter, { content: commandLit.noMutedUsers, flags: MessageFlags.Ephemeral }, 2);
