  ...
  run: async (client, inter) => {
    ...
    if (member.id === inter.user.id)
      return await reply(inter, {
        content: 'No puedes desmutearte a ti mismo',
        ephemeral: true,
        deleteTime: 2,
      });
    if (member.user.bot)
      return await reply(inter, {
        content: 'No puedes desmutear a un bot',
        ephemeral: true,
        deleteTime: 2,
      });
    ...
    if (!muteRole)
      return await reply(inter, {
        content: 'No existe el rol "Muted" -> no hay nadie muteado',
        ephemeral: true,
        deleteTime: 2,
      });
    ...
    if (!member.roles.cache.has(muteRole.id))
      return await reply(inter, {
        content: 'El miembro no está muteado',
        ephemeral: true,
        deleteTime: 2,
      });
    ...
    await reply(inter, {
      content: `El miembro **${member}** ha sido desmuteado`,
      ephemeral: true,
      propagate: false,
      deleteTime: 2,
    });
