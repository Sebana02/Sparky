...
module.exports = {
  ...
  run: async (client, inter) => {
    ...
    if (member.user.bot)
      return await reply(inter, {
        content: 'No puedes expulsar a un bot',
        ephemeral: true,
        deleteTime: 2,
      });
    if (!member.kickable)
      return await reply(inter, {
        content: 'No se pudo expulsar al miembro del servidor',
        ephemeral: true,
        deleteTime: 2,
      });
    ...
    let dmMessagge = await member
      .send(`Has sido expulsado del servidor **${inter.guild.name}** por **${reason}**`)
      .catch(() => null);

    //Kick the member
    await member.kick(reason).catch((error) => {
      // If this catch statement is reached, the member was not kicked, so we need to delete the DM we sent in case it was sent
    ...
    await reply(inter, {
      content: `**${member}** ha sido expulsado del servidor`,
      ephemeral: true,
      propagate: false,
      deleteTime: 2,
    });
