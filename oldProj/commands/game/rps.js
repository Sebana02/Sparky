...
class RPS {
    ...
    return await new Promise(async (resolve, reject) => {
      ...
      collector.on('collect', async (i) => {
        try {
          ...
          if (i.user === this.inter.options.getUser(literals.optionName) || i.user === this.inter.user) {
            ...
            elections.push({
              user: i.user,
              index: JSON.parse(i.customId).index,
            });
  ...
  showResult(elections, embed) {
    if (elections.length === 0)
      ...
      modifyEmbed(embed, {
        description:
          (elections[0].index + 1) % 3 === elections[1].index
