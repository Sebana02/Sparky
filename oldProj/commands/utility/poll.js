...
module.exports = {
  ...
  run: async (client, inter) => {
    ...
    const options = inter.options
      .getString(literals.optionsName)
      .split(',')
      .map((e) => e.trim())
      .filter(Boolean);
    if (options.length < 2)
      return await reply(inter, {
        content: literals.insufficientOptions,
        ephemeral: true,
        deleteTime: 2,
      });
    else if (options.length > 10)
      return await reply(inter, {
        content: literals.tooManyOptions,
        ephemeral: true,
        deleteTime: 2,
      });
...
function createButtons(votes) {
  ...
  const buttons = votes.map((vote, i) =>
    new ButtonBuilder().setLabel(vote.option).setCustomId(`vote_${i}`).setStyle('Primary')
  );
...
async function createCollection(inter, votes, poll, components) {
  ...
  const collector = msg.createMessageComponentCollector({
    filter,
    time: inter.options.getNumber(literals.timeName) * 1000,
  });
